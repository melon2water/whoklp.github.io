...
def single_song(song_id):
    ...
    media = None
    media = database.get_media(song_id)
    # Data integrity checks
    if song == None:
        song = []
...
def single_movie(movie_id):
    ...
    media = None
    media = database.get_media(movie_id)
...
def single_tvshowep(tvshowep_id):
    ...
    media = None
    media = database.get_media(tvshowep_id)
...
def single_genre(genre_id):
    ...
    #genre_value = None
   # genre_value = database.find_genre_id(genre_id)
    #genre_value_id = genre_value['md_id']

    genre = None
    genre = database.find_genre(genre_id)
    genre_name = genre[0]['md_type_name']
    gen_val = genre[0]['md_value']
    ...
    genre_list = None
    if (genre_name=='song genre'):
        genre_list = database.get_genre_songs(genre_id)
    elif (genre_name=='film genre'):
        genre_list = database.get_genre_movies_and_shows(genre_id)
    elif (genre_name=='podcast genre'):
        genre_list = database.get_genre_podcasts(genre_id)
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if (request.method == 'POST'):
            ...
            if ('artis_id' not in request.form):
                newdict['artis_id'] = 'Empty storage location'
            else:
                newdict['artis_id'] = request.form['artis_id']
                print("We have a value: ", newdict['artis_id'])
            ...
            # forward to the database to manage insert
            songs = database.add_song_to_db(newdict['song_title'], newdict['length'], newdict['storage_location'], newdict['artis_id'] ,newdict['description'], newdict['song_genre'])
